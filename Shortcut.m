function Shortcut(n)
%This is the function that will be used in the GUI when the user asks for the shortcut
%This code was developed with the help of the code Amaze by Cleve Moler obtained from Math Works
    ...
    %tree = A list of all the cells that have been visited
    %[i,j] = "B" nodes from which the barrier in between will be eliminated

n = 15; %Number of barrier nodes in a row
...
%% ==================================================Shortest Path=======================================
%This will only plot the direct path from the beginning node to the final one
...
while 1  
    ...
    if ~isempty(ngh) %This if statement will ocurr while there are neighbor cells available
        ...
        storage(end+1,:) = [c r]; %The current and next node are being added to the list of visited nodes 
...
%All the code down below from lines 89 to 94 is used to plot the path of the shortcut
    highlight(Cp,path, ...
        'edgecolor',cyan, ...
        'lineStyle','-', ...
        'nodecolor',cyan, ...
...
function [Bp,Cp] = plotboth(B,C,Ccolor) %This function serves to plot both the barriers and the cells
    ...
    k = (0:n^2-1)'; %All the lines from 133 to 140 serve to plot the variable "B" or the barriers 
    ...
    k = 1:m^2; %All the lines from 145 to 152 serve to plot the variable "c" or the cells 
